...
var TOKEN_NAME = "My Sunday Token";
var TOKEN_SYMBOL = "MSUT";
...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    let nm=await instance.name();
    let sy=await instance.symbol();
    await assert.equal(nm,TOKEN_NAME);
    await assert.equal(sy,TOKEN_SYMBOL)
...
it('lets 2 users exchange stars', async() => {
    ...
    let tst_name1 = await instance.lookUptokenIdToStarInfo(tokenId1);
    await assert.equal(name1,tst_name1);
    assert.equal(await instance.ownerOf.call(tokenId1), user1);

    let name2="Star 2";
    let tokenId2 = 102;
    await instance.createStar(name2, tokenId2, {from: user2});
    let tst_name2 = await instance.lookUptokenIdToStarInfo(tokenId2);
    await assert.equal(name2,tst_name2);
